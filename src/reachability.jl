...
@with_kw struct ReachabilitySolver{S} <: Solver
    reach::Set{S} = Set{S}()
    avoid::Set{S} = Set{S}()
    solver::Solver = ValueIterationSolver()# can use any solver that returns a value function :o # should not be able to use ValueIterationSolver... gamma = 1
end
...
# POMDPs.initialstate(r::Union{ReachabilityMDP, ReachabilityPOMDP}) = initialstate_distribution(r.problem)
