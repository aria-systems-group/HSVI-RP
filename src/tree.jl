...
struct SARSOPTree
    ...
    ba_children::Vector{Vector{Int}} #ba_children::Vector{UnitRange{Int}} # [ba_idx][o_idx] => bp_idx
    ...
    Γ_init::Vector{AlphaVec{Int}}
    effective_discount::Float64
    is_cycle::BitVector
    actions_sampled::Vector{Int}
    Nbao::Vector{Vector{Int}} # [ba_idx][o_idx] => N(b,a,o)
    Nb::Vector{Int} #[b_idx] => N(b)
    Nba::Vector{Vector{Int}} #[b_idx][a_idx] => N(b,a)
end

function SARSOPTree(solver, pomdp::POMDP)

    # println("Writing sparse tabular")
    sparse_pomdp = ModifiedSparseTabular(pomdp)
    # println("Finished writing sparse tabular")
...
POMDPs.discount(tree::SARSOPTree) = tree.effective_discount
...
function insert_root!(solver, tree::SARSOPTree, b)
    ...
    push!(tree.is_cycle, false)
    push!(tree.Nb, 0)
    push!(tree.Nba, zeros(Int, length(actions(tree))))
    fill_belief!(tree, 1)
    return tree
end
...
function update(tree::SARSOPTree, b_idx::Int, a, o)
    ...
    V̲, V̄ = if tree.is_terminal[bp_idx]
        ...
        lower_value(tree, tree.b[bp_idx]), upper_value(tree, tree.b[bp_idx]) #lower_value(tree, tree.b[bp_idx]), tree.V_upper[bp_idx] #
    end
...
function add_belief!(tree::SARSOPTree, b, ba_idx::Int, o)
    ...
    b_idx = findfirst(x->x==b, tree.b)
    if b_idx != nothing
        return b_idx, tree.V_lower[b_idx], tree.V_upper[b_idx]
    end
...
function fill_unpopulated!(tree::SARSOPTree, b_idx::Int)
    ...
    for a in A
        ...
        Q̲ = Rba
        push!(tree.poba, poba)
        # push!(tree.Nba, zeros(Int))
        push!(tree.Nbao, zeros(Int, N_OBS))
        for o ∈ O
            # belief update
