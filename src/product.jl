...
function POMDPs.reward(mdp::Union{ProductMDP, ProductPOMDP}, s::ProductState{S, Q}, a::A) where {S, A, Q}
    dist = transition(mdp, s, a)
    exp_r = 0.0
    for (sp, p) in weighted_iterator(dist)
        p == 0.0 ? continue : nothing
        exp_r += p*reward(mdp, s, a, sp)
     end
    return exp_r
end
...
POMDPs.discount(problem::Union{ProductMDP, ProductPOMDP}) = 1.0 #all problem.discount are undiscounted
...
function POMDPs.stateindex(problem::Union{ProductMDP, ProductPOMDP}, s::ProductState{S,Q}) where {S,Q} 
    if s == problem.sink_state 
        return length(states(problem))
    end
    si = stateindex(problem.problem, s.s)
    qi = s.q
    return LinearIndices((num_states(problem.automata), length(states(problem.problem))))[qi, si]
end
